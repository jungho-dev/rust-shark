...
macOS-first; Linux and Windows run the packet analyzer (the egress monitor
daemon is Unix-only). Fully offline — no cloud, no telemetry.
...
- **Dense terminal UI**: 256-color modern-dark theme, scrollable packet list,
  independently toggleable detail / hex panes
...
libpcap is required (the `rust-shark-core` engine links it; `rusqlite` is bundled).
...
This is a Cargo workspace: `rust-shark-core` (reusable engine) + `rust-shark`
(the CLI binary).
...
- **Linux**: `sudo rust-shark capture -i eth0`, or `sudo setcap cap_net_raw+eip ./rust-shark`
- **macOS**: `sudo rust-shark capture -i en0`, or add yourself to `access_bpf`
...
Optional `~/.config/rust-shark/config.toml`:
...
`F` flows, `T` timeline, `f` follow stream, `d` toggle detail pane, `x` toggle hex pane, `z` toggle
both panes, `Esc` close overlay, `q` quit. The packet list shows a scrollbar
when it overflows.
...
- **`rust-shark-core`** — capture, decoders, flow tracking + reassembly, filters,
  ...
  inspector reducer.
- **`rust-shark`** — the CLI binary and the two ratatui frontends (capture TUI,
  inspector).
...
Apache-2.0. GeoIP data, if fetched, is © db-ip.com under CC-BY-4.0.
