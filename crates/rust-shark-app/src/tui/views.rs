...
use rust_shark_core::decode::{DecodedPacket, Layer};
use rust_shark_core::flow::ReassemblyBuffer;
use rust_shark_core::flow::StreamData;

use super::{App, OverlayKind, theme};
        ...
        .map(|p| {
            Row::new(vec![
                Cell::from(Span::styled(
                    format!("\u{2605}{}", p.number),
                    Style::default().fg(theme::TEXT_STRONG),
                )),
                Cell::from(p.timestamp.format("%H:%M:%S%.3f").to_string()),
                Cell::from(format!("{} → {}", p.summary.source, p.summary.destination)),
                Cell::from(p.summary.info.chars().take(50).collect::<String>()),
            ])
        })
        .collect();
    let table = Table::new(
...
pub fn render_throughput(f: &mut Frame, area: Rect, app: &App) {
    ...
    let spark = Sparkline::default()
        .block(Block::default().style(theme::base()).title(format!(
            "Throughput {cur} pkt/s {}/s ({}s)",
            fmt_bytes(bps),
            data.len()
        )))
        .data(&data)
        .style(Style::default().fg(theme::ACCENT_TCP));
...
fn header(cols: &[&str]) -> Row<'static> {
    Row::new(cols.iter().map(|c| c.to_string()).collect::<Vec<_>>())
        .style(Style::default().fg(theme::HEADER).bold())
}
...
pub fn render_stream(f: &mut Frame, area: Rect, app: &App) {
    ...
    for chunk in printable(&data.client_to_server).split('\n') {
        lines.push(Line::from(Span::styled(
            format!("» {chunk}"),
            Style::default().fg(theme::ACCENT_TCP),
        )));
    }
    for chunk in printable(&data.server_to_client).split('\n') {
        lines.push(Line::from(Span::styled(
            format!("« {chunk}"),
            Style::default().fg(theme::ACCENT_UDP),
        )));
    }
