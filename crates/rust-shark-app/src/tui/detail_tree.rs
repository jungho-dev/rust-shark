...
use rust_shark_core::decode::{DnsRdata, Layer, TlsHandshakeMessage};
            ...
            if pkt.retransmission {
                all_lines.insert(
                    ...
                    Line::from(Span::styled(
                        "⚠ TCP Retransmission",
                        Style::default().fg(theme::ALERT).bold(),
                    )),
                );
            }
            all_lines
...
fn detail_lines(layer: &Layer) -> Vec<Line<'static>> {
    ...
    let header_style = Style::default().fg(theme::DETAIL_HEADER).bold();
    let field_style = Style::default().fg(theme::TEXT_DIM);
