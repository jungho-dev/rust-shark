...
fn ja4_alpn(first: Option<&str>) -> String {
    match first {
        Some(s) if !s.is_empty() => {
            let b = s.as_bytes();
            let first_byte = b[0];
            let last_byte = b[b.len() - 1];
            // Per the JA4 spec: if the first/last ALPN byte is not an ASCII
            // alphanumeric, substitute the pair with the lowercase hex of the
            // first byte's high nibble and the last byte's low nibble.
            if first_byte.is_ascii_alphanumeric() && last_byte.is_ascii_alphanumeric() {
                format!("{}{}", first_byte as char, last_byte as char)
            } else {
                let hi = first_byte >> 4;
                let lo = last_byte & 0x0f;
                format!("{hi:x}{lo:x}")
            }
        }
        _ => "00".to_string(),
