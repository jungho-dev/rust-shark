...
/// Maximum forward distance (in bytes) from `next_seq` to an incoming
/// segment's `seq` that is still treated as a plausible future segment.
///
/// Serial-number arithmetic (`seq_lt`) interprets any wrapping distance with
/// the high bit set as "behind", so a segment whose `seq` is ~2^31 ahead of
/// `next_seq` would be misread as already-consumed and silently dropped as a
/// `Duplicate`. On untrusted input such a `seq` is reachable with spoofed or
/// garbage sequence numbers. We therefore reject any segment whose forward
/// gap exceeds this window. The value (1 GiB) is far larger than any real
/// TCP receive window (max 1 GiB even with full window scaling, RFC 7323),
/// so it never rejects a legitimate in-window segment while still staying
/// well below the 2^31 sign-flip boundary.
const MAX_FORWARD_GAP: u32 = 1 << 30;
...
impl ReassemblyBuffer {
    ...
    pub fn insert_segment(&mut self, seq: u32, data: &[u8]) -> ReassemblyResult {
        ...
        // Reject implausibly far-future segments before the serial-number
        // classification below. A segment whose seq is strictly ahead of
        // next_seq but farther than MAX_FORWARD_GAP is bogus (spoofed/garbage
        // seq on untrusted input). Without this guard, a forward gap near 2^31
        // flips the sign of seq_lt and the far-future segment is silently
        // misclassified as a Duplicate. The `seq_lt(next_seq, seq)` guard keeps
        // legitimate backward duplicates/overlaps on their existing code path.
        if seq_lt(self.next_seq, seq) && seq.wrapping_sub(self.next_seq) > MAX_FORWARD_GAP {
