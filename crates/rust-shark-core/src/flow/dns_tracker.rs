...
pub struct DnsTracker {
    pending: LruCache<u16, PendingQuery>,
}
...
impl DnsTracker {
    pub fn new() -> Self {
        Self {
            pending: LruCache::new(NonZeroUsize::new(4096).unwrap()),
        }
    }
    ...
    pub fn observe(
        ...
        if !dns.is_response {
            let qname = dns
                ...
                .map(|q| q.qname.clone())
                .unwrap_or_default();
            // LruCache evicts the least-recently-used (oldest) pending query
            // when at capacity, preserving the freshest queries for pairing.
            self.pending.put(
                dns.transaction_id,
                PendingQuery {
                    packet_number,
            ...
            let q = self.pending.pop(&dns.transaction_id)?;
