...
fn record_rtt(state: &mut FlowState, is_low: bool, tcp: &TcpBits, ts: DateTime<Utc>) {
    ...
    if tcp.ack_flag {
        ...
        // Split off the still-unacked remainder (keys > ack), leaving only the
        // acked entries (keys <= ack) in `other`; restore the remainder after.
        // Avoids allocating a Vec of keys on every ACK in the hot path.
        let remainder = match ack.checked_add(1) {
            Some(next) => other.split_off(&next),
            None => Default::default(),
        };
        let mut srtt = state.srtt_ms;
        for send_ts in other.values() {
