...
fn extract_field(field: &FieldPath, layers: &[Layer]) -> Option<FilterValue> {
    ...
    for layer in layers {
        match layer {
            ...
            Layer::Dns(dns) if path == "dns.qname" => {
                return dns
                    .questions
                    .first()
                    .map(|q| FilterValue::Str(q.qname.clone()));
            }
            Layer::TlsClientHello(tls) if path == "tls.sni" => {
                return tls.sni.as_ref().map(|s| FilterValue::Str(s.clone()));
            }
            Layer::Icmp(icmp) => match path {
